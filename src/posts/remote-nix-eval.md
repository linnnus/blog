...
But even that can't save this poor computer.
Remote builds still require [instantiation] to happen locally
...
which does the evaluation on the remote as well.
I can't decide if it's brilliant or a giant hack
but either way [here it is][remote_build.sh].

When writing and iterating on the script,
I've been very careful to keep it as a linear sequence of steps
so I figure we can just walk through it now to explain how it works.
First up, some boilerplate:
...
whieh was arguably faster but felt waaay more hacky.
...
At long last we can activate the new configuration.
...
Now, the real `nixos-rebuild` does some more stuff when invocking `switch-to-configuration`.
In particular:
